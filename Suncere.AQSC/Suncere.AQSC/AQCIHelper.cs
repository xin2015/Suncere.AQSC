    ...
    public class AQCIHelper
    {
        ...
        private static void CalculateAQCI(IAQCICalculate data, Dictionary<string, decimal> IAQCIDic)
        {
            if (IAQCIDic.Count == 6)
            {
                ...
                data.PrimaryPollutant = string.Join(",", ParameterHelper.PollutantDic.Where(o => IAQCIDic.Where(t => t.Value == data.AQMI).Select(t => t.Key).Contains(o.Key)).Select(o => o.Value));
            }
        }
        ...
        private static void CalculateIAQCI(IAQCIReport data, Dictionary<string, decimal> IAQCIDic)
        {
            ...
            {
                IAQCIPropertiesDic[item.Key].SetValue(data, item.Value, null);
            }
        }
        #endregion
        #region 公共方法
